...
# FlashPortal V40.2 Upload + Payment Hotfix

Fixes:
- Upload form no longer requires `game_title`.
- Added Supabase schema patch to add missing columns and reload schema cache.
- Stripe payment buttons now redirect in the same tab to avoid popup issues.

Run this SQL in Supabase if uploads still fail:
supabase/v40_2_schema_cache_hotfix.sql

Commit summary:
Fix upload schema cache and payment redirects


# FlashPortal V40.3 Package/Game Fix

Fixes:
- Ensures `package.json` exists at the project root and includes Next.js.
- Adds Guess the Celebrity game files to `public/games/guess-the-celebrity/`.
- Adds `/play/guess-the-celebrity` route.
- Adds the game to the homepage game list.
- Includes Vercel root check instructions.
