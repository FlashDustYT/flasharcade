-- FlashPortal V61 REAL Profile / Follow Fix
-- Run ONCE in Supabase SQL Editor.
-- This creates the missing tables/functions the app actually calls.
...
drop index if exists user_profiles_username_lower_unique;
...
begin
  ...
  set
    followers = (select count(*) from public.profile_follows where following_id = target_user_id),
...
begin
  ...
  perform public.refresh_profile_follow_counts(me);
  perform public.refresh_profile_follow_counts(target_profile_id);

  select followers into follower_count
...
set
  followers = (select count(*) from public.profile_follows f where f.following_id = p.id),
