-- ============================================================
-- FlashPortal V80 BACKEND RUN ONCE
-- Fixes:
-- - Profile badge popout data support
-- - Mutual-follow-only direct messages
-- - Comment delete stays available to comment author and post owner
-- - How Many Rings achievement sync, including 10-ring Perfect Decade
-- ============================================================
...
begin
  ...
  limit 1;
  if c is not null then execute format('alter table public.achievement_catalog drop constraint %I', c); end if;
end $$;
...
('how_many_rings_dynasty','Dynasty Builder','rare','game','how-many-rings','Won at least 3 rings in a How Many Rings title window.','Win 3+ rings in How Many Rings. Any mode counts.',200,false),
('how_many_rings_perfect_decade','Perfect Decade','legendary','game','how-many-rings','Won all 10 rings in the 10-year title window.','Win 10 rings in How Many Rings. Any mode counts.',500,false),
...
begin
  ...
  if not exists(select 1 from public.achievement_catalog where code = badge_code) then
    raise exception 'unknown_badge';
  end if;

  insert into public.user_badges(user_id, badge_code)
  values(me, badge_code)
  on conflict(user_id, badge_code) do nothing;
...
-- Mutual-follow-only messaging. Existing conversations still work, but new ones require both users to follow each other.
...
begin
  ...
  if coalesce(mutual,false) = false then
    raise exception 'Messages only open when both people follow each other';
  end if;

  insert into public.conversations default values returning id into found_conversation;
  insert into public.conversation_members(conversation_id, user_id)
  values (found_conversation, me), (found_conversation, other_user_id)
...
begin
  ...
  where sc.id=target_comment_id
    and (sc.user_id=uid or exists(select 1 from public.social_posts sp where sp.id=sc.post_id and sp.user_id=uid));

  update public.social_posts sp
  set comments=(select count(*)::integer from public.social_comments sc where sc.post_id=sp.id and coalesce(sc.is_deleted,false)=false)
...
returns table(
  id bigint, user_id uuid, body text, image_url text, video_url text, media_type text,
  created_at timestamptz, likes integer, dislikes integer, comments integer, my_reaction text,
  author_username text, author_display_name text, author_avatar_url text, author_is_private boolean, author_last_seen_at timestamptz
)
language sql
security definer
set search_path = public
...
from (select post_id, count(*)::integer as total from public.social_post_reactions where reaction='like' group by post_id) counts
where sp.id = counts.post_id;

update public.social_posts sp
set dislikes = coalesce(counts.total,0)
from (select post_id, count(*)::integer as total from public.social_post_reactions where reaction='dislike' group by post_id) counts
where sp.id = counts.post_id;

update public.social_posts sp
set comments = coalesce(counts.total,0)
from (select post_id, count(*)::integer as total from public.social_comments where coalesce(is_deleted,false)=false group by post_id) counts
where sp.id = counts.post_id;

create index if not exists social_posts_feed_v80_idx on public.social_posts(created_at desc) where coalesce(is_deleted,false)=false and coalesce(is_private,false)=false;
create index if not exists social_post_reactions_user_post_v80_idx on public.social_post_reactions(user_id, post_id);
create index if not exists social_comments_post_v80_idx on public.social_comments(post_id, created_at) where coalesce(is_deleted,false)=false;
create index if not exists user_badges_user_v80_idx on public.user_badges(user_id, earned_at desc);
